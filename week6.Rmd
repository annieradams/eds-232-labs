...
fish_clean = read_csv(here( "data_fish", "fish_clean.csv")) %>% 
...
Today, we will look at preforming Random Forest and Boosted Trees on this data set. We will continue to come back to this data set as we learn more models in this section.
...
```{r, include=FALSE}
load(file = here::here( "data_fish",  "rf_tune.rda"))
```
...
```{r, include=FALSE}
load(file = here::here("data_fish",  "xgb_tune.rda"))
```
...
```{r}
rf_final = finalize_workflow(rf_workflow, select_best(rf_cv_tune, metric = "roc_auc"))
...
test_predict2_rf = predict(train_fit_rf, test, type = "prob") %>% #get prediction probabilities for test 
  bind_cols(test) %>%  #bind to testing column
  mutate(habitat = as.factor(habitat))
```

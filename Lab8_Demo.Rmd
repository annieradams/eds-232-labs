...
svm_lienar_fit <- svm_linear_final %>% fit(sim_data)
svm_linear_fit
```
...
svm_rbf_fit %>% 
  ...
  plot()
```
circles more in middle, triangles on outside, classifying on left side as one class, and classifying others as another class 

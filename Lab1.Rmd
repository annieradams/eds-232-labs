...
**This is not a useful plot. What surprises me is that there seems to be two different groups of price points for pumpkins, one hovering around \~20, and one hovering around \~75. I was surprised that there is such a distinct difference between these two groups, but perhaps it is because one is for organic pumpkins versus non organic pumpkins.**
...
**Of these three variables, the one most highly correlated with price is package. This makes sense as larger amounts of pumpkins cost more money, whereas smaller amounts of pumpkins would cost less money.**
...
**The main issue with fitting a linear regression to this data is that it over and underestimates for nearly all sizes of packages. Based on the graph, our data does not look linear, and therefore fitting a linear regression line to this data will not give us accurate predictions. It also seems as though package size 3 was in our training data, but not in our testing data at all.**
...
```{r}
save(pumpkins_train, file = "pumpkin_train.RData")
```

...
## R
```{r}
kaggle_dat <- read_csv(here::here("data_spotify", "genres_v2.csv"))
...
```{r}
doParallel::registerDoParallel()  #build trees in paralle
  #200s
system.time(
    tree_rs <- tune_grid(
      wf_tree_tune,
...
ggplot()+
  geom_boxplot(data = predict_data, aes(.pred_class, acousticness))
